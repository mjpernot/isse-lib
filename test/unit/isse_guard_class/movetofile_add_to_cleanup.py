...
"""Program:  movetofile_add_to_cleanup.py

    Description:  Unit testing of MoveToFile.add_to_cleanup in
        isse_guard_class.py.

    Usage:
        test/unit/git_class/movetofile_add_to_cleanup.py
...
class UnitTest(unittest.TestCase):
    ...
    def test_parse_xml_file_default(self):
        ...
        isse = isse_guard_class.MoveToFile(self.file_path, self.review_dir,
                                           self.dissem_dir)
        isse.add_to_cleanup(self.fname2)

        self.assertEqual((isse.cleanup_list), (self.cleanup_list))
